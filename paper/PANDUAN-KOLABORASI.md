...
- **Repo:** `https://github.com/Kruwpuck/gnn-marl-network-slicing`
- **Kelas dokumen:** IEEEtran **journal** (`\documentclass[lettersize,journal]{IEEEtran}`)
...
```bash
git clone https://github.com/Kruwpuck/gnn-marl-network-slicing.git
cd gnn-marl-network-slicing/paper
...
4. git add -A
5. git commit -m "pesan jelas, mis: tulis section System Model"
...
Bibliografi ada di **`paper/refs.bib`**. Saat ini isinya cuma **1 entri placeholder**
(`contoh2024`) supaya template ter-compile — ganti/hapus setelah ada referensi asli.
Ada dua cara mengelola:
...
```bibtex
@article{liuGraphNeural2023,
  author  = {Liu, Wei and others},
  title   = {Graph Neural Networks for Network Slicing},
...
```latex
Pendekatan ini terinspirasi \cite{liuGraphNeural2023}.
...
| `reference/` | Contoh & how-to IEEEtran | 📖 baca saja |
