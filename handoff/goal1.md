...
|---|---|---|---|
| A1 | Constraint mengikat pada policy terlatih | `\|violation_held-out − δ\| ≤ t₍0.975,n−1₎ · SE_seed` **dan** `t₍0.975,n−1₎ · SE_seed ≤ 1 std window` | latih baseline referensi (`ippo`) pada n ≥ 5 seed sampai konvergen; `SE_seed` = sd antar-seed / √n |
...
>
> Klausa kedua (`t·SE ≤ 1 std window`) ada karena bentuk `|x − δ| ≤ k·SE` sendirian **memberi hadiah pada instrumen yang lebih berderau**: makin kacau pengukuran, makin lebar pitanya, makin gampang lolos. Pembacaan greedy ronde 5 lolos klausa pertama justru karena `SE_seed`-nya 1.18 pp — lima kali lipat pembacaan stokastik. Batas presisi memakai satuan yang sudah dideklarasikan di A2a dan A2b (std window), bukan angka baru.
>
