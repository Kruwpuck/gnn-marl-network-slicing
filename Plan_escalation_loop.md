...
**Target**: 1 mesin (Windows 11, PowerShell 5.1 + Git Bash, GPU RTX 3060 lokal), 1 CLI (Claude Code)
**Prinsip**: eksekutor tidak riset. Perencana tidak ngoding. Handoff = file `.md`.

> **Environment: Windows.** Semua perintah ditulis untuk PowerShell kecuali ditandai
> `# git bash`. Terverifikasi di mesin ini (2026-08-04): `claude` 2.1.221,
> **tidak ada `tmux`**, flag `--effort` tersedia. Mesin ini sekaligus GPU host —
> tidak ada VPS, tidak ada SSH/WireGuard di jalur kerja ini.
...
```
PROJECT_ROOT     = ______________________   # path Windows, mis. C:\Users\<user>\Documents\exp-model
...
GPU_HOST         = localhost                # mesin ini sendiri (RTX 3060). Tidak ada host terpisah
PYTHON           = .venv\Scripts\python.exe # WAJIB. `python` sistem tidak punya gymnasium/torch
...
COMPACT_AT       = 150000                   # token terpakai sebelum /compact (lihat FASE 2b)
RUN_ID           = ______________________   # mis. 2026-08-04-run01
RESERVED_ENV     = ______________________   # AGENT DILARANG SENTUH
```

**Isi `RESERVED_ENV` dengan yang benar-benar tidak tergantikan.** Di repo ini itu berarti
`.venv\` (torch+PyG+rliable, versi terkunci — lihat HANDOVER §9) dan seluruh
`results\checkpoints\`, `results\logs\`, `results\eval\` (12 hari wall-clock GPU, tidak
bisa dibuat ulang). Agent boleh **membaca** semuanya; dilarang menulis/menghapus.
...
- [ ] `Get-Location`, `git status`, `git log --oneline -10`
- [ ] Petakan struktur repo (maks kedalaman 3): `Get-ChildItem -Recurse -Depth 2 -Directory`
- [ ] Catat: bahasa, package manager, cara jalankan test, cara jalankan training
- [ ] Cek `claude --version`. Cek skill: `claude` interaktif lalu `/plugin`, atau
      `Get-ChildItem $env:USERPROFILE\.claude\skills`, `Get-ChildItem $env:USERPROFILE\.claude\plugins\cache`
      — di mesin ini `~\.claude\skills` isinya cuma `learned`; skill nyata datang dari plugin
- [ ] Cek GPU luang: `nvidia-smi`. Training lain jalan = jangan mulai iterasi berat
- [ ] Cek venv hidup: `.venv\Scripts\python.exe -c "import torch; print(torch.cuda.is_available())"`
...
PowerShell tidak punya brace expansion — pakai loop:

```powershell
Set-Location $PROJECT_ROOT
"code","handoff","research","prompts","runs\$RUN_ID\transcripts" | ForEach-Object {
...
│   ├── BLOCKED.md
│   └── RESEARCH.md
├── prompts\                  # prompt per-peran, dibaca CLI (lihat FASE 6)
│   ├── exec.md
│   ├── brief.md
│   └── research.md
...
- Interpreter: <PYTHON>. Jangan pakai `python` sistem
- Training berat: cek `nvidia-smi` dulu, jangan tabrakan dengan run yang sedang jalan
...
Effort diatur lewat **flag CLI `--effort`**, bukan env var.

```powershell
claude -p "..." --model sonnet --effort low      # eksekutor
claude -p "..." --model opus   --effort medium   # perencana
```

> **Sudah diverifikasi di mesin ini** (`claude --version` = 2.1.221):
> ```
> --effort <level>    Effort level for the current session (low, medium, high, xhigh, max)
> ```
> Ini menggantikan `MAX_THINKING_TOKENS` yang jadi keraguan di draft plan sebelumnya —
> flag resmi ada, jadi jangan bergantung pada env var. Kalau versi CLI kamu naik dan
> flag ini hilang, jatuhkan ke instruksi prompt: eksekutor diminta "jangan analisis
> panjang, langsung eksekusi dan ukur".

---

## FASE 2b — COMPACT TIAP 150K TOKEN

Sesi panjang = context penuh = token mahal dan kualitas turun. Aturan:

**Ambang: `/compact` tiap ~150.000 token terpakai.**

### Sesi interaktif (kamu duduk di depan CLI)

| langkah | perintah |
|---|---|
| cek pemakaian | `/context` |
| padatkan | `/compact` |
| padatkan + arahkan | `/compact fokus ke STATE.json, STUCK.md, dan diagnosis terakhir` |

**Yang perlu diketahui, jujur:** agent **tidak bisa** memanggil `/compact` sendiri —
itu perintah harness (slash command), bukan tool. Tidak ada cara agent men-trigger-nya.
Jadi ambang 150k adalah **tugas manusia** atau auto-compact bawaan Claude Code. Yang
bisa dilakukan agent: memberi tahu kalau context sudah berat, lalu kamu yang mengetik.

**Compact di batas fase, bukan di tengah diagnosis.** Titik aman: sesudah `GATE`
dilewati, sesudah commit hijau, sesudah `BRIEF.md`/`RESEARCH.md` ditulis. Compact di
tengah debugging membuang jejak error yang belum sempat dipakai. Skill `strategic-compact`
(FASE 8) memang untuk memilih titik ini.

**Sebelum compact, pastikan state sudah di disk** — `STATE.json`, `ledger.md`, dan
handoff `.md` adalah memori sesungguhnya. Kalau ketiganya sudah ditulis, compact tidak
menghilangkan apa pun yang penting.

### Sesi headless (`claude -p`)

Tiap invokasi `claude -p` = context baru. Ambang 150k jarang tersentuh dalam satu
invokasi; kalau tersentuh, auto-compact bawaan yang menangani. Aturan 150k praktis
hanya berlaku untuk sesi interaktif panjang.

### Catat ke ledger

```
## <ISO8601> | lvl <n> | COMPACT | context <sebelum>k -> <sesudah>k | <sha>
```

Bertambahnya `COMPACT` ke daftar `AKSI` di FASE 7 disengaja: berapa kali sebuah run
perlu dipadatkan itu sendiri data penelitian tentang biaya loop.
