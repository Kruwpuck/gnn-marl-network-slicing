...
| `central-ppo` | timely_throughput_mbps | 65.0930 | **64.2197** | -0.8733 |
| `central-ppo` | sla_satisfaction_pct | 86.0790 | **85.0752** | -1.0038 |
| `central-ppo` | urllc_delay_p99 | 7.5337 | **8.0300** | +0.4963 |
| `central-ppo` | embb_p5_mbps | 1.6694 | **0.4887** | -1.1808 |
...
| `gnn-mappo_gat` | timely_throughput_mbps | 42.0703 | **68.0716** | +26.0013 |
| `gnn-mappo_gat` | sla_satisfaction_pct | 57.8433 | **90.0951** | +32.2519 |
| `gnn-mappo_gat` | urllc_delay_p99 | 6.2007 | **8.7739** | +2.5732 |
| `gnn-mappo_gat` | embb_p5_mbps | 1.5316 | **0.2781** | -1.2536 |
| `gnn-mappo_sage` | timely_throughput_mbps | 40.0205 | **68.5994** | +28.5789 |
| `gnn-mappo_sage` | sla_satisfaction_pct | 55.3232 | **90.7354** | +35.4122 |
| `gnn-mappo_sage` | urllc_delay_p99 | 6.1860 | **8.7439** | +2.5579 |
| `gnn-mappo_sage` | embb_p5_mbps | 1.1778 | **0.0431** | -1.1347 |
...
| `ippo` | timely_throughput_mbps | 66.8108 | **68.8698** | +2.0589 |
| `ippo` | sla_satisfaction_pct | 88.2183 | **90.9757** | +2.7574 |
| `ippo` | urllc_delay_p99 | 6.9704 | **8.2876** | +1.3172 |
| `ippo` | embb_p5_mbps | 0.0064 | **0.0000** | -0.0064 |
...
| `central-ppo` | 13.91 | 14.24 | 0.217 | 0.091 | 2.191 | 2.398 | 0.215 |
| `gnn-madqn_gat` | 15.41 | 15.22 | — | — | — | — | — (DQN: no action distribution) |
| `gnn-madqn_sage` | 14.16 | 14.04 | — | — | — | — | — (DQN: no action distribution) |
| `gnn-mappo_gat` | 36.24 | 11.18 | 0.200 | 0.091 | 2.193 | 2.398 | 0.202 |
| `gnn-mappo_sage` | 38.11 | 10.90 | 0.203 | 0.091 | 2.217 | 2.398 | 0.200 |
| `idqn` | 19.66 | 17.85 | — | — | — | — | — (DQN: no action distribution) |
| `ippo` | 14.54 | 11.13 | 0.202 | 0.091 | 2.291 | 2.398 | 0.198 |
| `mlp-knn-ppo` | 33.08 | 11.12 | — | — | — | — | — (not in the policy_confidence sweep) |

Among the algorithms with an action distribution, entropy spans only 0.100 nats (2.191-2.291, ceiling 2.398) while their throughput readout gap spans 29.45 Mbps (-0.87 to +28.58). The argmax is statistically degenerate to the same degree everywhere; whether that degeneracy wrecks the measured KPI is architecture-dependent. Entropy therefore does not predict which model the greedy readout will misrepresent — which is precisely why P3 gates on the stochastic readout rather than screening greedy numbers for plausibility.


Largest single-KPI readout gap in this wave: **28.58 Mbps** on `timely_throughput_mbps`. A gap that size is not a rounding difference between two ways of printing the same policy — it is the difference between measuring the policy and measuring its mode. Which architectures collapse and which do not is reported above as a finding, not filtered out.
